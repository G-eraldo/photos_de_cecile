    ...
    <div class="mx-auto max-w-6xl">
      <div class="grid items-start gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(360px,0.7fr)] lg:gap-18">
        <div class="lg:sticky lg:top-28 lg:self-start">
          <div class="overflow-hidden bg-[#ddd4c9] shadow-sm">
            <NuxtImg :src="`/images/bons-cadeaux/bon-cadeau-${selectedPhotos}-photos.jpg`" alt="Aperçu du bon cadeau personnalisé" class="h-full w-full object-cover" sizes="sm:100vw lg:55vw" />
          </div>
          <p class="mt-3 text-center text-xs text-[#806957]">Le bon personnalisé sera envoyé au format PDF après paiement.</p>
        </div>
        ...
        <div class="max-w-xl p-7!">
          ...
          <p class="mt-8 leading-7 text-[#6d5b4e]">Choisissez une prestation, le forfait photo et ajoutez votre message. Le bon cadeau sera complété puis envoyé par e-mail après le paiement.</p>

          <div class="mt-8 border-y border-[#d8cec1] py-7">
            <h2 class="font-playfair text-xl">Pour quelle prestation ?</h2>
            <div class="mt-4 grid gap-2">
              <button v-for="prestation in prestations" :key="prestation.value" type="button" class="border p-4 text-left text-sm transition" :class="selectedPrestation === prestation.value ? 'border-[#503d30] bg-[#f5eeea]' : 'border-[#b9aa9c] hover:border-[#503d30]'" @click="selectedPrestation = prestation.value">
                {{ prestation.label }}
              </button>
            </div>
          </div>
          ...
          <div class="mt-8">
            ...
            <div class="mt-3 flex flex-wrap gap-2">
              <button v-for="choice in photoChoices" :key="choice.photos" type="button" class="rounded-full border px-4 py-2 text-sm transition" :class="selectedPhotos === choice.photos ? 'border-[#503d30] bg-[#503d30] text-white' : 'border-[#b9aa9c] hover:border-[#503d30]'" @click="selectedPhotos = choice.photos">
          ...
          <div class="mt-6">
            ...
            <div class="mt-3 grid gap-2">
              <button v-for="choice in deliveryChoices" :key="choice.value" type="button" class="border p-4 text-left transition" :class="delivery === choice.value ? 'border-[#503d30] bg-[#f5eeea]' : 'border-[#b9aa9c] hover:border-[#503d30]'" @click="delivery = choice.value">
                <span class="block text-sm font-medium">{{ choice.label }}</span><span class="mt-1 block text-xs leading-5 text-[#806957]">{{ choice.description }}</span>
              </button>
            </div>
          </div>
          ...
          <form class="mt-10 border-t border-[#d8cec1] pt-8" @submit.prevent="submitGiftCard">
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2">
              <div class="grid gap-2"><Label for="gift-nom">Votre nom</Label><Input id="gift-nom" v-model="nom" autocomplete="family-name" required /></div>
              <div class="grid gap-2"><Label for="gift-prenom">Votre prénom</Label><Input id="gift-prenom" v-model="prenom" autocomplete="given-name" required /></div>
            </div>
            <div class="mt-4 grid gap-2"><Label for="gift-beneficiaire">Prénom du bénéficiaire</Label><Input id="gift-beneficiaire" v-model="beneficiaire" required /></div>
            <div class="mt-4 grid gap-2"><Label for="gift-message">Votre message <span class="text-[#806957]">(facultatif)</span></Label><Textarea id="gift-message" v-model="message" :maxlength="250" rows="4" placeholder="Un petit mot pour accompagner ce cadeau…" /></div>
            <div class="mt-4 grid gap-2"><Label for="gift-email">Votre e-mail</Label><Input id="gift-email" v-model="email" type="email" autocomplete="email" required /></div>
            <div v-if="delivery === 'courrier'" class="mt-4 grid gap-4 sm:grid-cols-2">
              <div class="grid gap-2 sm:col-span-2"><Label for="gift-rue">Numéro et rue</Label><Input id="gift-rue" v-model="rue" autocomplete="street-address" required /></div>
              <div class="grid gap-2"><Label for="gift-code-postal">Code postal</Label><Input id="gift-code-postal" v-model="codePostal" inputmode="numeric" autocomplete="postal-code" required /></div>
              <div class="grid gap-2"><Label for="gift-ville">Ville</Label><Input id="gift-ville" v-model="ville" autocomplete="address-level2" required /></div>
            </div>
            <Button type="submit" class="mt-6 w-full" :disabled="paymentPending"><Mail class="mr-2 h-4 w-4" />{{ paymentPending ? 'Redirection vers le paiement…' : `Payer ${formattedTotal} €` }}</Button>
          </form>
        </div>
      </div>
    </div>
