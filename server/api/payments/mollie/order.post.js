...
const validateCartItem = async (config, item) => {
  ...
  if (
    ...
    throw createError({
      statusCode: 400,
      statusMessage: "Un tirage du panier est invalide.",
    });
  }

  const product = await findProductForOrder(config, {
    productId: item?.productId,
    slug: item?.slug,
  });
  const availableFormats = product.tarifs_formats || product.formats || [];
  if (
    !availableFormats.some((entry) =>
      typeof entry === "string"
        ? entry === format.trim()
        : entry?.format === format.trim(),
    )
  ) {
    throw createError({
      statusCode: 400,
  ...
  if (
    Object.keys(product.options || {}).some(
  ...
  const photo = await inspectPrivateUpload(
    verifyPrivateUploadToken(item?.uploadToken),
  );
  const tariff = Array.isArray(product.tarifs_formats)
    ? product.tarifs_formats.find((entry) => entry?.format === format.trim())
    : null;
  const formatPrice = Number(tariff?.prix ?? product.prix_a_partir_de);
  const hasFringedEdges = Object.entries(options).some(
    ([name, value]) =>
      /bord|finition/i.test(name) && /frang/i.test(String(value)),
  );
  const fringeFee = hasFringedEdges
    ? Number(product.supplement_bords_franges ?? 1)
    : 0;
  const deliveryFee = Number(product.supplement_courrier ?? 5);
  if (
    !Number.isFinite(formatPrice) ||
    formatPrice < 0 ||
    !Number.isFinite(fringeFee) ||
    fringeFee < 0 ||
    ...
    throw createError({
      statusCode: 400,
      statusMessage: "Le tarif du tirage est invalide.",
    });
  }
...
export default defineEventHandler(async (event) => {
  ...
  if (
    ![nom, prenom, email, adresse].every((field) => isText(field)) ||
  ...
  if (body?.delivery !== "courrier") {
    throw createError({
      statusCode: 400,
      statusMessage: "Les tirages sont expédiés uniquement par courrier.",
    });
  }
  ...
  if (!Array.isArray(items) || !items.length || items.length > 20) {
    throw createError({
      statusCode: 400,
      statusMessage: "Le panier est invalide.",
    });
  }

  const config = getMollieConfig();
  const validatedItems = await Promise.all(
    items.map((item) => validateCartItem(config, item)),
  );
  const quantity = validatedItems.reduce(
  ...
  if (quantity > 20)
    throw createError({
      statusCode: 400,
      statusMessage: "Le panier ne peut pas contenir plus de 20 tirages.",
    });
  ...
  const deliveryFee = Math.max(
    ...validatedItems.map((item) => item.deliveryFee),
  );
  ...
  const orderDetails = {
    ...
    produit:
      validatedItems.length === 1
        ? validatedItems[0].product
        : `${quantity} tirages photo`,
    produitSlug:
      validatedItems.length === 1
        ? validatedItems[0].productSlug
        : "panier-tirages",
    format:
      validatedItems.length === 1
        ? validatedItems[0].format
        : "Commande groupée",
    options: { réception: `Par courrier (+${deliveryFee.toFixed(2)} €)` },
    quantite: quantity,
    prixUnitaire:
      validatedItems.length === 1 ? validatedItems[0].unitPrice : subtotal,
    fraisLivraison: deliveryFee,
    items: validatedItems.map(({ photo, ...item }) => ({
      ...item,
      photoNom: photo.filename,
    })),
  };
