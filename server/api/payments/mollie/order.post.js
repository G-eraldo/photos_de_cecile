...
export default defineEventHandler(async (event) => {
  ...
  if (![nom, prenom, email, adresse, format].every((field) => isText(field)) || !/^\S+@\S+\.\S+$/.test(email.trim())) {
    throw createError({ statusCode: 400, statusMessage: "Merci de renseigner vos coordonnées et l’adresse de livraison." });
  }
  if (!Number.isInteger(quantity) || quantity < 1 || quantity > 20) {
    throw createError({ statusCode: 400, statusMessage: "La quantité doit être comprise entre 1 et 20." });
  }
  if (!options || Array.isArray(options) || typeof options !== "object") {
    throw createError({ statusCode: 400, statusMessage: "Les options de tirage sont invalides." });
  }

  const config = getMollieConfig();
  const product = await findProductForOrder(config, { productId, slug });
  if (!Array.isArray(product.formats) || !product.formats.includes(format.trim())) {
  ...
  const uploadedPhoto = await inspectPrivateUpload(verifyPrivateUploadToken(details?.uploadToken));
  const reference = `c${randomUUID().replace(/-/g, "")}`;
  const price = Number(product.prix_a_partir_de);
  const total = Number((price * quantity).toFixed(2));
  const orderDetails = { nom: nom.trim(), prenom: prenom.trim(), email: email.trim(), adresse: adresse.trim(), produit: product.titre, produitSlug: product.slug, format: format.trim(), options, quantite: quantity, prixUnitaire: price, photoNom: uploadedPhoto.filename };
  const order = await createStoredOrder(config, { reference, details: orderDetails, montant_total: total, mollie_payment_id: `pending_${reference}`, statut: "en_attente", photo_privee: uploadedPhoto });

  try {
    const payment = await createMolliePayment(config, { amount: total.toFixed(2), reference, description: `Commande — ${product.titre} × ${quantity}`, confirmationPath: "/tirages-photo/confirmation", paymentType: "commande" });
