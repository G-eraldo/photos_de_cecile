...
export async function sendOrderConfirmation({ reference, details, total }) {
  ...
  const isCourierGift = isGift && details.options?.réception?.startsWith("Par courrier");
  let customerEmailSent = true;
  try {
    const invoice = await generateOrderInvoicePdf({ reference, details, total });
    const voucher = isGift && !isCourierGift ? await generateGiftVoucherPdf({ details }) : null;
