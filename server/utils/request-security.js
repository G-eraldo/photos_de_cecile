...
export async function enforceRateLimit(event, { scope, limit, windowMs, global: shared = false }) {
  ...
  if (redisUrl && redisToken) {
    try {
      ...
      if (payload.error || !Array.isArray(payload.result)) throw new Error('rate_limit_invalid');
      [count, remainingMs] = payload.result.map(Number);
      if (!Number.isFinite(count) || !Number.isFinite(remainingMs) || remainingMs < 0) throw new Error('rate_limit_invalid');
    } catch {
      throw createError({ statusCode: 503, statusMessage: 'Le service est momentanément indisponible. Veuillez réessayer.' });
    }
  } else {
    if (process.env.NODE_ENV === 'production') {
      throw createError({ statusCode: 503, statusMessage: 'La protection du service doit être configurée.' });
    }
    const now = Date.now();
    for (const [oldKey, entry] of buckets) if (entry.resetAt <= now) buckets.delete(oldKey);
    if (!buckets.has(key)) {
      if (buckets.size >= maximumBuckets) throw createError({ statusCode: 503, statusMessage: 'Veuillez réessayer dans quelques instants.' });
  ...
  if (count > limit) {
    setResponseHeader(event, 'Retry-After', String(Math.max(1, Math.ceil(remainingMs / 1000))));
...
export function enforceTrustedOrigin(event) {
  ...
  if (!origin) return;
  const allowedOrigins = [process.env.SITE_URL, ...(process.env.NODE_ENV !== 'production' ? ['http://localhost:3000'] : [])].filter(Boolean);
  if (!allowedOrigins.some((allowed) => origin === String(allowed).replace(/\/$/, ''))) {
